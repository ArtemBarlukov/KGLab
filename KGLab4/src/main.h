...
void Main_Init()
{
    ...
    Load_Texture("img/Spraytlist.png", &sprite, GL_CLAMP, GL_CLAMP, GL_NEAREST);
...
void Hero_Show(Hero *obj)
{
    ...
    // Проверка коллизий с левой стеной
    if (obj->x < 0) {
        obj->dx = 0;  // Остановка движения по горизонтали
        obj->x = 0;   // Установка положения на левой стене
    }
